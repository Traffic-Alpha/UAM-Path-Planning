...
class ACEnvWrapper(gym.Wrapper):
    ...
    def get_scaled_snir(self):  # 获取每个大格的均值，形成新的snir矩阵
        ...
        scale_grid_num = int(self.speed / self.resolution)  # 一个大格里包含多少个小格，大格的边长是速度
    ...
    def state_wrapper(self, state, actions, infos):
        ...
        for ac_idx, (aircraft_id, aircraft_info) in enumerate(aircraft.items()):
            ...
            else: # 如果snir值小于阈值，则让飞机留在原地, 返回并执行相反的动作
            ...
            uncertainty_matrix = self.get_around_uncertainty(int(y_scale), int(x_scale))
            ...
            uncertainty_attr = uncertainty_matrix.copy()
            ...
            feature_set = {
                ...
                "sinr_attr": sinr_attr,
                "uncertainty_attr": uncertainty_attr,
            }

        return feature_set, None

    def reward_wrapper(self, states, infos, dones) -> Tuple[float, bool]:
        """自定义 reward 的计算
        """
        reward = 0
        ...
        for ac_idx, (aircraft_id, aircraft_info) in enumerate(aircraft.items()):
            aircraft_pos = aircraft_info["position"]
            # aircraft_pos = [aircraft_pos[0] - self.x_min, aircraft_pos[1] - self.y_min,
            #                 aircraft_pos[2]]  # 转换坐标系到以地图的左下角为原点
            ...
            if self.uncertainty_matrix[y_scale, x_scale] == 0:
                new_area_reward = 0.2
                reward += new_area_reward
            else:
                new_area_reward = - (1 * self.uncertainty_matrix[y_scale, x_scale])
                reward += new_area_reward
    ...
    def reset(self, seed=1) -> Tuple[Any, Dict[str, Any]]:
        ...
        state, _ = self.state_wrapper(state=state, actions={"drone_1": (0, 0)}, infos={"step_time": 0})
    ...
    def step(self, actions: Dict[str, int]) -> Tuple[Any, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        feature_set, update_action = self.state_wrapper(state=states, actions=new_actions, infos=infos)  # 处理 state
        ...
        if update_action is not None:
            ...
            feature_set, update_action = self.state_wrapper(state=states, actions=new_actions, infos=infos)  # 处理 state
            if isinstance(feature_set, str) :  # 如果回退一步依然在sinr低的区域
                raise TypeError("Return to low SINR area, check the postion")

        new_rewards, new_dones = self.reward_wrapper(states=states, infos=infos, dones=dones)  # 处理 reward

        if new_dones:
            self.actru_trajectory = self.ac_history_pos

        return feature_set, new_rewards, new_dones, new_dones, infos
