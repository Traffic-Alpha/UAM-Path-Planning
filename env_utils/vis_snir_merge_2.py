...
def render_map(
        ...
        trajectories_rl, trajectories_TSP, trajectories_PDPCC, trajectories_straight, figure_step, goal_points, speed,
        snir_threshold, img_path, figure_type, save_forbidden_area=False
):
    fig, ax = plt.subplots()
    ...
    if figure_type == 'step':
        ...
        plot_trajectories_sinr(ax, trajectories_rl, trajectories_TSP, trajectories_PDPCC, compare_flag=figure_type,
                               trajectories_straight=trajectories_straight)  # 绘制 aircraft 的轨迹
        plot_goal_points(ax, start_points, color='#1e90ff', marker=">", point_type="S")  # 绘制乘客起点
        plot_goal_points(ax, end_points, color='#1e90ff', marker="o", point_type="D")  # 绘制乘客终点
...
def plot_snir_binary(ax, x_min, y_min, x_max, y_max, threshold, grid_z):
    ...
    """
    # 计算 x 和 y 坐标
    grid_z_masked = np.where(grid_z <= threshold, -25, grid_z)
    cmap = ListedColormap(['black', 'white'])
    bounds = [-25, threshold, 15]
    ...
    # 添加颜色条
    cbar = plt.colorbar(cax, ax=ax, ticks=[-25, threshold, 15])
    cbar.ax.set_yticklabels(['-25', str(threshold), '15'])
...
def plot_trajectories_sinr(ax, trajectories_rl, trajectories_TSP, trajectories_PDPCC, compare_flag="exp", trajectories_straight=None):
    """绘制车辆轨迹信息
    """
    path_rl = list(trajectories_rl.values())[0]
    path_TSP = list(trajectories_TSP.values())[0]
    path_PDPCC = list(trajectories_PDPCC.values())[0]
    path_straight = list(trajectories_straight.values())[0]
    # 提取 x 和 y 坐标
    x_coords_rl, y_coords_rl = zip(*path_rl)
    x_coords_TSP, y_coords_TSP = zip(*path_TSP)
    x_coords_PDPCC, y_coords_PDPCC = zip(*path_PDPCC)
    ...
    x_coords_stright, y_coords_stright = zip(*path_straight)
    # 绘制轨迹的第一个点
    ax.scatter(x_coords_rl[0], y_coords_rl[0], s=140, c='#1e90ff', marker='*')
    ...
    # 绘制轨迹
    if compare_flag=="com": # experiment=exp: RL+straight, compare=com: TSP+PDPCC
        ax.plot(x_coords_PDPCC, y_coords_PDPCC, c=color[0], label="PDPCC")
        # ax.plot(x_coords_TSP, y_coords_TSP, c=color[2], label="CPTSP", linewidth=3, linestyle='--')
        # 绘制轨迹的最后一个点
        ax.scatter(x_coords_PDPCC[-1], y_coords_PDPCC[-1], s=180, c=color[0], marker='*')
        ...
        ax.scatter(x_coords_rl[-1], y_coords_rl[-1], s=140, c=color[1], marker='*')
        # ax.scatter(6472.0455749867515, 8320, s=50, c=color[1], marker='.')
        # ax.scatter(6368.940116587175, 8960, s=50, c=color[1], marker='.')
        # ax.scatter(3840, 5274.0560841261895, s=50, c=color[1], marker='.')
        # ax.scatter(4160, 5324.932398597897, s=50, c=color[1], marker='.')
        # ax.scatter(6400, 2466.6666666666665, s=50, c=color[1], marker='.')
        # ax.scatter(7360, 2442.051282051282, s=50, c=color[1], marker='.')
